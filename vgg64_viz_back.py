...
for ii in range(0, args.epochs):
    ...
    for jj in range(0, len(train_filenames), args.batch_size):
        ...
        if (jj % (100 * args.batch_size) == 0):
            ...
            f.close()
            '''
            for kk in range(len(_bp_back)):
                back = _bp_back[kk]['dout']
                if len(np.shape(back)) == 4:
                    print (kk)
            '''
            '''
            for kk in range(len(_bp_back)):
                ...
                if len(np.shape(back)) == 4:
                    dedf = np.sign(back)
                    img = scipy.misc.imresize(dedf[0], 4.)
                    plt.imsave('%d_%d_bp.jpg' % (jj, kk), img)

            for kk in range(len(_lel_back)):
                back = _lel_back[kk]['dout']                    
                if len(np.shape(back)) == 4:
                    dedf = np.sign(back)
                    img = scipy.misc.imresize(dedf[0], 4.)
                    plt.imsave('%d_%d_lel.jpg' % (jj, kk), img)
            '''

            img00 = scipy.misc.imresize(np.sign(_bp_back[1]['dout'][0, :, :, 0]), 4.0)
            img01 = scipy.misc.imresize(np.sign(_bp_back[4]['dout'][0, :, :, 0]), 8.0)
            img02 = scipy.misc.imresize(np.sign(_bp_back[7]['dout'][0, :, :, 0]), 16.0)
            img03 = scipy.misc.imresize(np.sign(_bp_back[10]['dout'][0, :, :, 0]), 32.0)

            img10 = scipy.misc.imresize(np.sign(_lel_back[1]['dout'][0, :, :, 0]), 4.0)
            img11 = scipy.misc.imresize(np.sign(_lel_back[4]['dout'][0, :, :, 0]), 8.0)
            img12 = scipy.misc.imresize(np.sign(_lel_back[7]['dout'][0, :, :, 0]), 16.0)
            img13 = scipy.misc.imresize(np.sign(_lel_back[10]['dout'][0, :, :, 0]), 32.0)

            img0 = np.concatenate((img00, img01, img02, img03), axis=1) 
            img1 = np.concatenate((img10, img11, img12, img13), axis=1)
