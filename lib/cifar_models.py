...
def cifar_conv(batch_size, dropout_rate, init='alexnet', num_classes=10):
    l1 = ConvBlock(input_shape=[batch_size, 32, 32, 3], filter_shape=[5, 5, 3, 64],  strides=[1,1,1,1], init=init, name='block1')
    l2 = AvgPool(size=[batch_size, 32, 32, 64], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")

    l3 = ConvBlock(input_shape=[batch_size, 16, 16, 64], filter_shape=[5, 5, 64, 96], strides=[1,1,1,1], init=init, name='block2')
    l4 = AvgPool(size=[batch_size, 16, 16, 96], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")

    l5 = ConvBlock(input_shape=[batch_size, 8, 8, 96], filter_shape=[5, 5, 96, 128], strides=[1,1,1,1], init=init, name='block3')
    l6 = AvgPool(size=[batch_size, 8, 8, 128], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")

    l7 = ConvToFullyConnected(input_shape=[batch_size, 4, 4, 128])
    l8 = FullyConnected(input_shape=4*4*128, size=num_classes, init=init, bias=0.1, name='fc1')
