...
class SparseFC(Layer):
    ...
    def __init__(self, size, num_classes, init_weights, alpha, activation, bias, last_layer, rate, l2=0., name=None, load=None, train=True, fa=False):
        ...
        self.num = int(self.rate * np.prod(self.size))
        ...
        self.fb_val = tf.Variable(val, dtype=tf.float32)

        self.weights = tf.SparseTensor(indices=self.idx, values=self.val, dense_shape=self.size)
        self.fb = tf.SparseTensor(indices=self.idx, values=self.fb_val, dense_shape=self.size)

        # w = np.random.uniform(low=-1.0/sqrt_fan_in, high=1.0/sqrt_fan_in, size=self.size)
    ...
    def backward(self, AI, AO, DO):
        ...
        if self.fa:
            DI = tf.sparse_tensor_dense_matmul(self.fb, tf.transpose(DO))
            DI = tf.transpose(DI)
        else:
