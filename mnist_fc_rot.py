...
def to_spike_train(mat):
    ...
    rot15 = sp.ndimage.rotate(input=mat, angle=15., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    rot30 = sp.ndimage.rotate(input=mat, angle=30., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    rot45 = sp.ndimage.rotate(input=mat, angle=45., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    rot60 = sp.ndimage.rotate(input=mat, angle=60., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    rot75 = sp.ndimage.rotate(input=mat, angle=75., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    rot90 = sp.ndimage.rotate(input=mat, angle=90., axes=[1, 2], reshape=False).reshape(N, 1, H, W, C)
    
    mat = np.concatenate((rot15, rot30, rot45, rot60, rot75, rot90), axis=1)
...
l6 = SpikingSum(input_shape=[args.batch_size, args.time_steps, 64], init=args.init, activation=Relu(), name="ss1", train=True)
...
for ii in range(args.epochs):
    ...
    for jj in range(0, TRAIN_EXAMPLES, args.batch_size):
        ...
        xs = np.reshape(xs, (args.batch_size, args.time_steps, 28*28*1))
