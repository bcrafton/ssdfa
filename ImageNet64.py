...
acts, derivs, gvs = model.bp(X=features, Y=labels)
...
for ii in range(args.epochs):
    ...
    for jj in range(0, len(train_filenames), args.batch_size):
        [_total_correct, _total_top5, _, act, deriv] = sess.run([total_correct, total_top5, train, acts, derivs], feed_dict={handle: train_handle, batch_size: args.batch_size, dropout_rate: args.dropout, lr: lr_decay})
